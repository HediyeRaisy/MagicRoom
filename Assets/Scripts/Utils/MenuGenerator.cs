...
public class MenuGenerator : MonoBehaviour
{
    ...
    private void Start()
    {
        if (GameSetting.instance != null)
        {
            ...
            MagicRoomManager.instance.ExperienceManagerComunication.getGameConfigurationFromEM();
            MagicRoomManager.instance.ExperienceManagerComunication.getPlayers();
            ...
            if (propertyname != ThemePropertyName)
            {
                ...
                if (p.PropertyType == typeof(string))
                {
                    foreach (Attribute a in attrs)
                    {
                        ...
                        else if (a.GetType().ToString() == "PropertyReferenceFolders")
                        {
                            PropertyReferenceFolders prf = a as PropertyReferenceFolders;
                            g = GameObject.Instantiate(Resources.Load("UI/PropertyMenuBlockDropdown") as GameObject, transform.GetChild(0));
                            List<string> ls = new List<string>();
                            foreach (string fld in prf.folder)
                            {
                                foreach (string f in Directory.GetFiles(Application.streamingAssetsPath + "/" + fld))
                                {
                                    string file = f.Split('/').Last().Split('\\').Last();
                                    if (file.EndsWith(prf.extension))
                                    {
    ...
    public void PlayGame()
    {
        ...
        MagicRoomManager.instance.ExperienceManagerComunication.sendCommandGameReadyForMaximization();
    }
